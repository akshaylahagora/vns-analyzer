...
<style>
    ...
    .class-card { background-color: #ffffff; padding: 15px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); margin-bottom: 12px; border-left: 6px solid; border: 1px solid #eee; border-left: 6px solid; }
    ...
    .chg-green { color: #008000; font-weight: bold; }
    .chg-red { color: #d63031; font-weight: bold; }
...
SECTOR_MAP = { "NIFTY": "Index", "RELIANCE": "Energy", "HDFCBANK": "Banking", "TCS": "IT", "MARUTI": "Auto", "SUNPHARMA": "Pharma", "ITC": "FMCG", "BAJFINANCE": "Finance", "TATASTEEL": "Metal", "ULTRACEMCO": "Cement", "LT": "Infra" } # Truncated for brevity
FNO_STOCKS = [
...
FNO_STOCKS_LIST = list(set(FNO_STOCKS))
...
with st.sidebar:
    ...
    st.divider()
    c1, c2 = st.columns(2)
    view_min = c1.number_input("Min Price", 0, value=1000) 
    view_max = c2.number_input("Max Price", 0, value=100000)
...
def run_full_scan():
    ...
    for i, stock in enumerate(FNO_STOCKS_LIST):
        ...
        if df is not None:
            ...
            if close > 0: 
                results.append({ "Symbol": stock, "Sector": SECTOR_MAP.get(stock, "Other"), "Price": close, "Change": chg, "Category": cat, "Signal": sig, "History": hist, "BU": res, "BE": sup, "Trend": tr })
...
elif do_scan is True: st.info("Auto-Scanning..."); current_data = run_full_scan(); st.rerun()
else: current_data = payload

# --- DISPLAY ---
if current_data:
    st.caption(f"Last Scanned: {current_data['date']} {current_data['last_updated']} | Duration: {current_data.get('duration_label')}")
    st.divider()
    ...
    elif category_filter == "Atak Only": cats = [("⚠️ Atak Teji", atak_teji, "b-atak-top"), ("🛡️ Atak Mandi", atak_mandi, "b-atak-bot")]
    ...
    def show_details(s):
        ...
        c1.metric("Trend", s['Trend']); c2.metric("Res", f"{s['BU']:.2f}"); c3.metric("Sup", f"{s['BE']:.2f}")
        st.divider()
        def color(row):
            t = row['Type']
        ...
        # FIX: Explicitly cast to float before formatting
        h = pd.DataFrame(s['History'])
        for c in ["Open", "High", "Low", "Close"]: h[c] = pd.to_numeric(h[c])
            
        st.dataframe(
            h.style.apply(color, axis=1).format({"Open": "{:.2f}", "High": "{:.2f}", "Low": "{:.2f}", "Close": "{:.2f}"}),
            column_config={"Type": None, "BU": st.column_config.TextColumn("BU"), "BE": st.column_config.TextColumn("BE")},
