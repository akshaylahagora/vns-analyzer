...
def fetch_stock_data(symbol, start_date):
    try:
        ...
        headers = { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
            "Accept": "*/*", "Accept-Language": "en-US,en;q=0.9", "Referer": f"https://www.nseindia.com/get-quotes/equity?symbol={safe_symbol}" }
...
def classify_stock(df):
    ...
    for i in range(1, len(df)):
        ...
        if i == len(df) - 1:
            signal_desc = signal
            if "BREAKOUT" in signal: category = "Highly Bullish" # Logic mapping adjustment
            elif "ATAK (Top)" in str(bu): category = "Atak (Teji Side)"
            elif "ATAK (Bot)" in str(be): category = "Atak (Mandi Side)"
...
def check_auto_scan():
    ...
    try:
        ...
        if data.get('stocks') and len(data['stocks']) > 0 and 'Trend' not in data['stocks'][0]: return True, "Schema Update"
        return False, data
...
def show_details(item):
    ...
    def color_rows(row):
        s = row['Type']
        if s == 'bull_dark': return ['background-color: #228B22; color: white; font-weight: bold']*len(row) # Dark Green
        if s == 'bear_dark': return ['background-color: #8B0000; color: white; font-weight: bold']*len(row) # Dark Red
        if s == 'bull_light': return ['background-color: #90EE90; color: black; font-weight: bold']*len(row) # Light Green
        if s == 'bear_light': return ['background-color: #FFC0CB; color: black; font-weight: bold']*len(row) # Light Red
        return ['']*len(row)

    hist_df = pd.DataFrame(item['History'])
    if not hist_df.empty:
        st.dataframe(
            hist_df.style.apply(color_rows, axis=1).format({"Open": "{:.2f}", "High": "{:.2f}", "Low": "{:.2f}", "Close": "{:.2f}", "BU": "{:.2f}", "BE": "{:.2f}"}, na_rep=""),
            column_config={"Type": None}, use_container_width=True, height=400
        )
    else: st.write("No history data.")

if current_data:
    st.caption(f"Last Scanned: {current_data['date']} {current_data['last_updated']} | Duration: {current_data.get('duration_label', 'Unknown')}")
    st.divider()
